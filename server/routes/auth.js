...
const dbRun = (query, params) => {
  if (isProduction) {
    let i = 0;
    const pgQuery = query.replace(/\?/g, () => `$${++i}`);
    return pool.query(pgQuery, params).then(r => r);
  }
  return new Promise((resolve, reject) => {
...
const dbInsert = async (query, params) => {
  ...
  return new Promise((resolve, reject) => {
    db.run(query, params, function(err) {
      if (err) reject(err);
      else resolve({ lastID: this.lastInsertId });
    });
  });
};
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.json({ token, user: { id: result.lastID, full_name, email, role: 'user' } });
  } catch (err) {
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await dbRun('UPDATE password_reset_tokens SET used = 1 WHERE user_id = ?', [user.id]);
    await dbInsert(
